...
int *longeurdeschemins(graph *G,int *M /**matrice des longeurs*/,int u /**indice de sommet de depart*/){
	int i,j,*T=(int*)malloc(sizeof(int)),k,n=G->n;
for(k=0;k<n;k++)
	for(i=0;i<n;i++){
        for(j=0;j<n;j++){
...
/**l'algorithme de djikstra pour trouver le plus court cheemin entre deux sommet*/
void *djikstra(graph *G,int *M /**matrice des longeurs*/,int u /**indice de depart*/,int v /**indice d'arrive*/){
	int i,j,*T=(int*)malloc(sizeof(int)),k,n=G->n;
	int precedent[n];/**les sommet precedent a chaque sommet */
	for(k=0;k<n;k++)precedent[k]=u;
for(k=0;k<n+1;k++)
	for(i=0;i<n;i++){
        for(j=0;j<n;j++){
		...
		int *chemin=(int*)malloc(j*sizeof(int));
		i=v;
		k=0;
		while(j!=k){k++;
		chemin[j-k]=i;
		...
		printf("\n\t le plus court chemin entre %c et %c est :\n\n\t\t",charindice(G,u),charindice(G,v));
		for(i=0;i<j;i++)printf("%c \t ",charindice(G,chemin[i]));
		printf("\n\t\t de longeur:\t %d",T[v]);
		free(M);
}
